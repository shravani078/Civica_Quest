  ...
  const guides = {
    ...
    portfolio: {
      ...
      steps: [
        ...
        "See your civic skill scores",
        "Track achievements and badges",
        "Get AI-powered assessment",
        "Share your civic journey"
      ],
      tip: "Your portfolio tracks everything — play more games to build it up!"
    }
  };
...
// ================================================================
// Override cqChatbotReply to support multilingual responses via Claude API
const _originalCqChatbotReply = cqChatbotReply;

async function cqChatbotReplyML(userText) {
  const lang = (window.CQ_LANG && window.CQ_LANG.currentLang !== 'en')
    ? window.CQ_LANG.currentLang : null;

  // If English, use original local logic
  if (!lang) return _originalCqChatbotReply(userText);

  const langInfo = window.CQ_LANG && window.CQ_LANG.LANGUAGES[lang];
  const langName = langInfo ? langInfo.name : 'Hindi';

  // Try Claude API for multilingual response
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `You are CivicaQuest AI, an assistant for an Indian civic education platform for Class 9-12 students. 
The user is asking in or wants a response in ${langName}.
Keep proper nouns like EVM, VVPAT, Lok Sabha, Rajya Sabha, CivicaQuest unchanged.
Give a helpful, concise response about Indian civics, democracy, constitution, or platform features.
Respond ONLY in ${langName} language. Keep response under 200 words.
...
User question: ${userText}`
    ...
    if (response.ok) {
      ...
      return data.content?.[0]?.text || _originalCqChatbotReply(userText);
    }
  } catch(e) {
    // Fallback to local
  }

  return _originalCqChatbotReply(userText);
}

// Patch the send function to use async ML version
const _origSend = window.cqOpenChatbot;
window.cqOpenChatbot = function() {
  _origSend();

  // Patch after chatbot is created
  setTimeout(() => {
    const sendBtn = document.getElementById('cq_chat_send');
    const input   = document.getElementById('cq_chat_text');
    const log     = document.getElementById('cq_chat_log');

    if (!sendBtn || sendBtn.getAttribute('data-ml-patched')) return;
    sendBtn.setAttribute('data-ml-patched', '1');

    // Override send to use async ML version
    async function sendML() {
      const text = input.value.trim();
      if (!text) return;
      ...
      typing.className = 'cq-bot';
      typing.innerHTML = '<span style="opacity:0.5">⚙️ Thinking...</span>';
      log.appendChild(typing);
      log.scrollTop = log.scrollHeight;

      const reply = await cqChatbotReplyML(text);
      typing.style.whiteSpace = 'pre-line';
      typing.textContent = reply;
      log.scrollTop = log.scrollHeight;

      if (CQ.state.elderMode) cqSpeak(reply);
      if (Math.random() < 0.3) cqAddXP(5, 'Learning');
    }

    sendBtn.onclick = sendML;
    input.onkeydown = (e) => { if (e.key === 'Enter') sendML(); };

    // Update greeting in current language
    if (window.CQ_LANG && window.CQ_LANG.currentLang !== 'en') {
      const greeting = window.CQ_LANG.t('ai.greeting');
      if (greeting && log.children.length > 0) {
        log.children[0].style.whiteSpace = 'pre-line';
        log.children[0].textContent = greeting;
      }
      const inputEl = document.getElementById('cq_chat_text');
      if (inputEl) inputEl.placeholder = window.CQ_LANG.t('ai.placeholder');
    }
  }, 200);
};
